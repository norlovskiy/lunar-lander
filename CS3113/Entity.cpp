...
Entity::Entity(Vector2 position, Vector2 scale, const char *textureFilepath, 
    Status entityStatus) : mPosition {position}, mVelocity {0.0f, 0.0f}, 
    mAcceleration {0.0f, 0.0f}, mScale {scale}, mMovement {0.0f, 0.0f}, 
    mColliderDimensions {scale}, mTexture {LoadTexture(textureFilepath)}, 
    mTextureType {SINGLE}, mDirection {RIGHT}, mAnimationAtlas {{}}, 
    mAnimationIndices {}, mFrameSpeed {0}, mSpeed {DEFAULT_SPEED}, 
    mAngle {0.0f}, mStatus {entityStatus} { }

Entity::Entity(Vector2 position, Vector2 scale, const char *textureFilepath, 
        TextureType textureType, Vector2 spriteSheetDimensions, std::map<Direction, 
        std::vector<int>> animationAtlas, Status entityStatus) : 
        ...
        mSpeed { DEFAULT_SPEED }, mStatus {entityStatus} { }
...
void Entity::checkCollisionY(Entity *collidableEntities, int collisionCheckCount)
{
    for (int i = 0; i < collisionCheckCount; i++)
    {
        ...
        if (isColliding(collidableEntity))
        {
            ...
            mCollisionStatus = collidableEntity->mStatus;
...
{
    if ((other->mStatus == DISABLED)) return false;
    ...
    int collisionCheckCount)
{
    ...
    resetColliderFlags();
    if (mStatus != PLAYER) mVelocity.x = mMovement.x * mSpeed;            
...
// Add force dependent on angle
void Entity::addForcePolar(float magnitude) {
    const float angRad = mAngle * (std::numbers::pi / 180.0f); // Convert angle to radians
    mAcceleration.x += magnitude * std::sin(angRad);
    mAcceleration.y += magnitude * -(std::cos(angRad));
}
