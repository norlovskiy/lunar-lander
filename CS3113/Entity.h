...
enum Status { PLAYER, TERRAIN, PAD, DISABLED, NONE};
...
private:
    ...
    Status mStatus = DISABLED;
    Status mCollisionStatus = NONE;
...
public:
    ...
    Status      getCollisionStatus()       const { return mCollisionStatus;        }
    bool        getHit()                 const { return mIsHit;                }
    ...
    void setAngle(float newAngle) 
        { mAngle = newAngle;                       }
    void setStatus(Status entityStatus)
        { mStatus = entityStatus;                  }

    void addForce(Vector2 newForce) { 
        mAcceleration.x += newForce.x; 
        mAcceleration.y += newForce.y; 
    }

    void addForcePolar(float magnitude);
}; 
